...
async def s(ctx, *args):
    ...
    if (subreddit=='a'):
        ...
        scrape = reddit.webauth(reddit.auth, reddit.data, reddit.headers, args[0])
        res = reddit.searchposts(scrape, reddit.keywords_a)
        print(res)
        for found in res:
            out += found + "\n"
        ...
        scrape = reddit.webauth(reddit.auth, reddit.data, reddit.headers, args[0])
        res = reddit.searchposts(scrape, reddit.keywords_b)
        print(res)
        for found in res:
            out += found + "\n"
        length = str(len(res)) + " Results Found"
